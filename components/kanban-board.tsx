...
const STATUS_CONFIG = {
  TODO: { label: "Yapılacak", color: "bg-yellow-50 border-yellow-200", dot: "bg-yellow-400", header: "text-yellow-700" },
  IN_PROGRESS: { label: "Devam Ediyor", color: "bg-blue-50 border-blue-200", dot: "bg-blue-400", header: "text-blue-700" },
  DONE: { label: "Tamamlandı", color: "bg-green-50 border-green-200", dot: "bg-green-400", header: "text-green-700" },
} as const;
      ...
      <div className="grid grid-cols-3 gap-4 h-full">
        {(Object.keys(STATUS_CONFIG) as Status[]).map((status) => {
          ...
          return (
            <div
              key={status}
              className={`rounded-2xl border-2 transition-all ${cfg.color} ${isOver ? "ring-2 ring-indigo-400 ring-offset-1 scale-[1.01]" : ""}`}
            ...
            >
              <div className="px-4 py-3 flex items-center gap-2 border-b border-current border-opacity-10">
                ...
                <span className={`text-sm font-semibold ${cfg.header}`}>{cfg.label}</span>
                <span className={`ml-auto text-xs font-medium px-2 py-0.5 rounded-full bg-white bg-opacity-60 ${cfg.header}`}>
                  {columnTasks.length}
                </span>
              </div>
              ...
              <div className="p-3 space-y-2 min-h-[200px]">
                ...
                {isOver && dragging && (
                  <div className="h-16 rounded-xl border-2 border-dashed border-indigo-300 bg-indigo-50 flex items-center justify-center text-xs text-indigo-400">
                    Buraya bırak
                  </div>
                )}
              </div>
...
function KanbanCard({
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      onClick={onClick}
      className={`bg-white rounded-xl border border-gray-200 px-3 py-3 cursor-grab active:cursor-grabbing hover:border-indigo-300 hover:shadow-sm transition-all select-none ${
    ...
    >
      ...
      {showAssignee && task.assignee && (
        <p className="text-xs text-gray-400 mt-1.5 truncate">{task.assignee.name}</p>
      )}
    </div>
  );
}
